...
public class PathLogController implements Initializable {
    @FXML
    private AnchorPane rootPathLog;
    @FXML
    private Label pathLog;
    private Alert alert = new Alert(Alert.AlertType.NONE);

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            pathLog.setText(ConnectionUtil.getPathDetails(DayTableController.getRecordID()));
    ...
    public void logOut(ActionEvent actionEvent) {
        ...
        if (alert.getResult() == ButtonType.OK) {
            try {
                ...
                Main.switchSceneByFading(newScene,rootPathLog);
                return;
            } catch (IOException e) {
                e.printStackTrace();
            }
